...
def generate_launch_description():
    ...
    # ('/dlio/odom_node/odom') by default. In sim there is no dlio node -
    # Gazebo's DiffDrive plugin publishes odometry on '/odom' instead - so
    # remap the subscription to '/odom' when running with sim:=true. On real
    # hardware (sim:=false) this remaps the topic to itself, i.e. a no-op.
    odom_topic_remap = PythonExpression([
        "'/odom' if '", sim, "' == 'true' else '/dlio/odom_node/odom'"
    ])
