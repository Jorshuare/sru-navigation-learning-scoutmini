...
# Adapted for Scout Mini (2-DOF differential drive [v, omega]) - was 16/2576/3 for
# B2W's 3-DOF [vx, vy, omega]. last_action is now 2, not 3; everything else
# (depth embedding shape, LSTM hidden size) is unchanged - same VAE encoder weights,
# same SRU architecture, just genuinely fewer action dimensions.
STATE_DIM = 15  # linear_vel(3) + angular_vel(3) + gravity(3) + last_action(2) + target_pos_log(4)
DEPTH_EMBEDDING_DIM = 2560  # 64 * 5 * 8 (flattened VAE output)
POLICY_INPUT_DIM = STATE_DIM + DEPTH_EMBEDDING_DIM  # 2575 - confirmed against the
# real exported ONNX model's own obs input shape (1, 2575), not just computed.
LSTM_HIDDEN_DIM = 512
ACTION_DIM = 2  # [v, omega] - genuinely 2-DOF, confirmed against the exported
# ONNX model's own actions output shape (1, 2).
...
class LearningModel:
    ...
    Model Architecture:
        ...
        Navigation Policy (LSTM-based):
            - Inputs:
                - obs: [batch, 2575] (15 state features + 2560 depth embedding)
                - h_in: [1, batch, 512] (LSTM hidden state)
                - c_in: [1, batch, 512] (LSTM cell state)
            - Outputs:
                - actions: [batch, 2] - [v, omega], real units (m/s, rad/s), NOT
                  tanh-squashed - the exported policy's raw mean output must be
                  clamped to real limits by the caller (see predict() below),
                  matching DifferentialDriveAction's own processing during training.
                - h_out: [1, batch, 512]
                - c_out: [1, batch, 512]
    """
    ...
    def __init__(self, preprocess_model_path, policy_model_path):
        ...
        # Real hard limits (AgileX CAN protocol, PLAN.md sec 6/7 decision 6) - the
        # exported policy outputs real-unit [v, omega] directly (no tanh), so this
        # replaces B2W's POLICY_SCALE (which rescaled a tanh-squashed [-1,1] output).
        self._max_linear_velocity = constants.MAX_LINEAR_VELOCITY
        self._max_angular_velocity = constants.MAX_ANGULAR_VELOCITY
    ...
    def predict(
        ...
        Args:
            ...
            last_action: Previous action [v, omega] - 2-dim, Scout Mini is
                genuinely 2-DOF differential drive, no lateral component.
        ...
        Returns:
            Tuple of (velocity command [v, omega], raw action, target vector in base frame)
        """
        ...
        # State: linear_vel(3) + angular_vel(3) + gravity(3) + last_action(2) + target_pos_log(4) = 15
        ...
        # Clamp to real hardware limits - NOT tanh+scale. The exported policy's raw
        # actor-mean output is already in real units (m/s, rad/s), matching exactly
        # what DifferentialDriveAction.process_actions() does during training
        # (scale*raw+offset then clamp - scale/offset default to identity, so this
        # reduces to a direct clamp). Confirmed via a real inference smoke test on
        # the exported ONNX model, not assumed.
        cmd_vel = raw_action.copy()
        cmd_vel[:, 0] = np.clip(cmd_vel[:, 0], -self._max_linear_velocity, self._max_linear_velocity)
    ...
    def fake_run_once(self):
        ...
        print(f'Predicted cmd_vel: linear_x={cmd_vel[0]:.4f}, angular_z={cmd_vel[1]:.4f}')
...
class NavigationPolicyNode(Node):
    def __init__(
        ...
        self.prev_cmd = np.zeros(2)  # For low-pass filtering - [v, omega], no lateral term
        ...
        self.cmd_vel_ratio = 1.0  # SIM-ONLY: bypass joystick deadman switch
    ...
    def odom_callback(self, odom_msg: Odometry):
        ...
        # Throttled position printout (10x/sec) - diagnostic only. Fast enough
        # to catch sub-second goal reaches; still far below raw odom rate.
        self.get_logger().info(
            f'Robot position [{self.robot_frame_id}]: '
            f'x={self.robot_pos_w[0]:.3f}, y={self.robot_pos_w[1]:.3f}, z={self.robot_pos_w[2]:.3f}',
    ...
    def generate_cmd_vel(self):
        ...
        if is_arrived or self.is_abort_goal:
            ...
            if self.robot_odom_time - self.joy_time > self.system_delay:
                pass  # SIM-ONLY: bypassed joystick timeout
            ...
            twist = Twist()
            # Policy command (no joystick) - [v, omega], no lateral term. Scout
            # Mini is a differential-drive robot and physically cannot strafe,
            # unlike B2W - dropped LATERAL_VELOCITY_SCALE entirely rather than
            # keep a dead scale factor for a DOF this robot doesn't have.
            model_cmd = np.array([
                cmd[0].item() * self.cmd_vel_ratio,
                cmd[1].item() * self.cmd_vel_ratio
            ])
            ...
            # Add joystick inputs without filtering. twist.linear.y is left as
            # joystick-only (no policy contribution) - preserved as-is rather than
            # zeroed outright, since joystick teleport is a separate, already-
            # validated control path (Part A) that this change isn't meant to touch.
            twist.linear.x = float(filt_model[0]) + self.joy_linear_x
            twist.linear.y = self.joy_linear_y
            twist.angular.z = float(filt_model[1]) + self.joy_angular_z
    ...
    def _check_goal_reached(self, target_pos_w, robot_pos_w):
        ...
        if dist > self.arrive_goal_threshold:
            ...
            self.get_logger().info(
                f'Arrived: dist_to_goal={dist:.3f}m <= arrive_goal_threshold='
                f'{self.arrive_goal_threshold:.3f}m (robot=[{robot_pos_w[0]:.3f}, '
                f'{robot_pos_w[1]:.3f}], goal=[{target_pos_w[0]:.3f}, {target_pos_w[1]:.3f}])'
            )
    ...
    def _reset_last_action(self):
        ...
        Returns:
            Zero action vector [0, 0] - [v, omega], 2-dim for Scout Mini.
        """
