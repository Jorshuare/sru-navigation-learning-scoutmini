...
# Pre-defined configs
##
from navigation_template.terrains import SCOUT_MINI_MAZE_TERRAIN_CFG  # isort: skip
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        terrain_generator=SCOUT_MINI_MAZE_TERRAIN_CFG,
    ...
    raycast_camera = RayCasterCameraCfg(
        ...
        debug_vis=False,
        max_distance=11.0,
        # NOTE: upstream builds this via `PinholeCameraPatternCfg.from_ros_camera_info(
        # fx=72.7025, fy=72.7025, cx=94.4457, cy=62.5424, width=192, height=120,
        # downsample_factor=3)` - that classmethod does not exist in this server's
        # installed IsaacLab v2.1.1 (confirmed by grepping the source; a real
        # version mismatch between what this repo was written against and what's
        # installed here, not something to silently paper over). Reproduced the
        # identical result using the classmethod that IS available
        # (`from_intrinsic_matrix`) by pre-applying the same fx/fy/cx/cy /= 3,
        # width/height //= 3 scaling `from_ros_camera_info`'s docstring/behavior
        # implies (downsample_factor scales the raycast resolution down from the
        # real camera_info sample's resolution, not a separate post-process step -
        # confirmed since PinholeCameraPatternCfg's width/height fields are the
        # actual raycasting resolution). Values below are 72.7025/3, 94.4457/3,
        # 62.5424/3, 192//3, 120//3 - unchanged from upstream, not re-derived.
        pattern_cfg=patterns.PinholeCameraPatternCfg.from_intrinsic_matrix(
            intrinsic_matrix=[24.234166666666667, 0.0, 31.4819, 0.0, 24.234166666666667, 20.847466666666666, 0.0, 0.0, 1.0],
            width=64,
            height=40,
        ),
    )
